	...
	private function DrawJsFunctions()
	{
		...
		echo "
			...
			{			
				...
				document.location.href = '/en/account/accountmanagers/index?action='+action+'&view_type='+view_type+'&year='+year+'&month='+month+'&day='+day;		
			}
		";
		echo "</script>";
	...
	private function DrawMonth()
	{
		...
		echo "<tr>";
			echo "<th colspan='7'>";
				...
				echo " <a href=\"/account/accountmanagers/index?action=view&view_type=monthly&year=".$this->prevMonth['year']."&month=".$this->ConvertToDecimal($this->prevMonth['mon'])."&day=".$this->arrParameters['day']."\"> &laquo;&laquo; </a>";
				echo $this->arrParameters['month_full_name']." - ".$this->arrParameters['year'];
				echo " <a href=\"/account/accountmanagers/index?action=view&view_type=monthly&year=".$this->nextMonth['year']."&month=".$this->ConvertToDecimal($this->nextMonth['mon'])."&day=".$this->arrParameters['day']."\"> &raquo;&raquo; </a>";
				echo "</th>".$this->crLt;
				echo "<th class='tr_navbar_right'>				
		...
		if($max_empty_days < 7){
			...
			for($i = $max_empty_days+1; $i <= 7; $i++){
				...
				if (($actday == $this->arrToday['mday']) && ($this->arrToday['mon'] == $this->arrParameters["month"])) {
					...
					$counts = T3GoogleParser::count();
					$tasks = "<br/>";
					foreach ($counts as $count)
					{
						$webmasterAgent = T3Users::getUserById($count['assigned_to']);
						if ($webmasterAgent)
						{
							$tasks.= "<a href='http://t3leads.com/en/account/tasks/list?status=open&user=$webmasterAgent->id&SubmitButton=1' style='color: white; font-size:7pt'>".$webmasterAgent->nickname." ".$count['cnt']."</a><br/>";
						}	
					}
					...
					$class = " class='td'";
					$currentDate = $this->arrParameters["year"]."-".$this->arrParameters["month"]."-".$actday;
					$closedTasks = T3GoogleParser::countClosed($currentDate);
					
					if (isset($closedTasks) && !empty($closedTasks))
					{
						$tasks = "<br/>Closed Tasks<br/>";
						foreach($closedTasks as $closedTask)
						{
							$webmasterAgent = T3Users::getUserById($closedTask['assigned_to']);
							if ($webmasterAgent)
							{
								$tasks.= $webmasterAgent->nickname." ".$closedTask['cnt']."<br/>";
							}						
						}
					}
					else
					{
						...
						{
							$tasks = "<br/>No Tasks<br/>";
						}
					}
				} 
				echo "<td$class>$actday $tasks</td>".$this->crLt;
			}
			echo "</tr>".$this->crLt;
		}
			...
			for ($i=0; $i<7;$i++){
				...
				if (($actday == $this->arrToday['mday']) && ($this->arrToday['mon'] == $this->arrParameters["month"])) {
					...
					foreach ($counts as $count)
					{
						...
						if ($webmasterAgent)
						{
							$tasks.= "<a href='/en/account/tasks/list?status=open&user=$webmasterAgent->id&SubmitButton=1' style='color: white; font-size:7pt'>".$webmasterAgent->nickname." ".$count['cnt']."</a><br/>";
						}	
					}
